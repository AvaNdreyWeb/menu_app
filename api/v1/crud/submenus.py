...
def get_all(db: Session, menu_id: str):
    db_submenu_list = db.query(models.Submenu).filter(
        models.Submenu.menu_id == menu_id
    )
    submenu_dto_list = []
    for db_submenu in db_submenu_list:
        dishes_count = len(db_submenu.dishes)
        submenu_dto = schemas.SubmenuDTO(
            id=db_submenu.id,
            title=db_submenu.title,
            description=db_submenu.description,
            dishes_count=dishes_count
        )
        submenu_dto_list.append(submenu_dto)
    return submenu_dto_list


def get_by_id(db: Session, menu_id: str, submenu_id: str):
...
def create(db: Session, menu_id: str, submenu_data: schemas.CreateSubmenuDTO):
